...
// src/subprocess-coverage/merge.ts). src/index.ts and
// src/subprocess-coverage/index.ts stay excluded as pure re-export
// barrels, exercised end-to-end by the bin-test workspace package;
// provider.ts runs only in the vitest main process (that is what
// customProviderModule loads), which no collector in the tree can see;
// and the inactive platform twin can never execute on the OS running
// the gate.
const inactiveTwin =
  process.platform === "win32" ? "src/mock-bin.ts" : "src/mock-bin-windows.ts";

// Modules that execute only inside spawned children. Their coverage
// arrives solely through the merged child profiles, so a run that did
// not opt in must keep excluding them instead of gating them on zeros.
const childRunModules = [
...
const config = vitestPreset({
  ...
  // excludes.
  coverage: {
    // Extends the stock v8 provider with the child-profile merge; see
    ...
    exclude: [
      ...
      "src/subprocess-coverage/provider.ts",
      inactiveTwin,
      ...(propagationOn ? [] : childRunModules),
    ],
    // Threshold groups, not a global floor: a global aggregates every
    // included file, and the child-run modules would drag it below 80
    // merely by existing. The in-process pool keeps the 80% floors —
    // the brace keeps the gate platform-symmetric, matching whichever
    // twin this OS executes — while the child-run modules carry lower
    // floors of their own: raw NODE_V8_COVERAGE profiles are
    // best-effort (functions a child never calls are never even
    // compiled, children that are killed never flush a profile), and
    // mock-bin-runtime.ts keeps Windows-only helpers that cannot
    // execute on a Linux gate (and vice versa). The floors sit below
    // the observed numbers with headroom; the point is that these
    // files are measured at all, so regressions in what children
    // execute show up.
    thresholds: {
